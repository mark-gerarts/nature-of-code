(defpackage :nature-of-code.forces.exercise-6
  (:export :start-sketch)
  (:use :cl :trivial-gamekit)
  (:import-from :cl-bodge :vector-length :normalize))
(in-package :nature-of-code.forces.exercise-6)
...
(defmethod initialize-instance :after ((mover mover) &key)
  ...
  (setf (width  mover) (* 8 (mass mover)))
  ;; Spawn somewhere random, but at maximum height.
  (setf (location mover) (vec2 (random *width*) (- *height* (width mover)))))
...
(defmethod drag ((mover mover) (liquid liquid))
  (let* ((v (velocity mover))
         ...
         (drag-magn (* drag-magn (* (width mover) 0.1)))
         (drag v)
         (drag (mult drag -1))
         (drag (normalize drag))
...
(defmethod check-edges ((mover mover))
  (let* ((location (location mover))
    ...
    (when (< left 0)
      (progn
        (setf (x (location mover)) 0)
        (reverse-direction mover 'x)))
    (when (> right *width*)
      (progn
        (setf (x (location mover)) (- *width* w))
        (reverse-direction mover 'x)))
    (when (< bottom 0)
      (progn
        (setf (y (location mover)) 0)
        (reverse-direction mover 'y)))
    (when (> top *height*)
      (progn
        (setf (y (location mover)) (- *height* w))
        (reverse-direction mover 'y)))))
...
(defmethod display ((mover mover))
  (draw-rect (location mover) (width mover) (width mover)
             :fill-paint *gray*
             :stroke-paint *black*
             :thickness 2))
