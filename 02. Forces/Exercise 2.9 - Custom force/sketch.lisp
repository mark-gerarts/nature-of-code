(defpackage :nature-of-code.forces.exercise-9
  (:export :start-sketch)
  (:use :cl :trivial-gamekit)
  (:import-from :cl-bodge :vector-length :normalize))
(in-package :nature-of-code.forces.exercise-9)
...
(defmethod attract ((attractor attractor) (mover mover))
  (let* ((force (subt (location attractor) (location mover)))
         ...
         (strength (/
                    ...
                    (expt distance 2)))
         (attraction (mult (normalize force) strength)))
    ;; Repel close movers, while still attracting distant ones.
    (if (< (vector-length force) 40)
        (mult attraction -5)
        attraction)))
